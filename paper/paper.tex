...
Sin una confusion matrix formal generada sobre un split de test independiente, el análisis de errores se basa en los pares de clases con mayor riesgo de confusión por similaridad visual: leche descremada y leche entera (envases de la misma marca con diferente etiqueta), y aceite 1L y aceite 4L (misma marca, diferente tamaño). En los resultados de validación, estas clases mantuvieron mAP@0.5 aceptable ($> 0.7$), lo que sugiere que el modelo distingue atributos de tamaño y variaciones sutiles en el color del envase. La clase fideos, con el mAP más bajo, requeriría mayor cantidad de imágenes de entrenamiento para mejorar.
...
El mAP@0.5 de 0.924 obtenido en escenas multi-producto supera el umbral mínimo de 0.70 definido en la planificación del proyecto. Si bien la comparación directa con el dataset RPC~\cite{wei2019rpc} (que reporta mAP entre 0.50 y 0.80 sobre 200 categorías) no es directa por la diferencia de escala, los resultados confirman la viabilidad del enfoque para un catálogo acotado. La mejora al pasar de escenas individuales a multi-producto (+5.4 puntos de mAP@0.5) refuerza que la diversidad contextual en el entrenamiento es más determinante que la cantidad bruta de imágenes.
...
\begin{itemize}
  ...
  \item \textbf{Sin split de test independiente:} Las métricas reportadas corresponden al conjunto de validación. Un split de test separado permitiría una estimación más robusta de la capacidad de generalización.
  \item \textbf{Evaluación cuantitativa limitada:} No se generó una confusion matrix formal ni se midieron métricas de sistema (order-level accuracy, duplicate rate) sobre un benchmark estandarizado.
\end{itemize}
