...
Los avances recientes en detección de objetos en tiempo real, impulsados por la familia de arquitecturas YOLO~\cite{redmon2016yolo, redmon2018yolov3, wang2024yolov10}, han democratizado el acceso a modelos de alta precisión y baja latencia ejecutables en hardware de bajo costo. En el ámbito de productos de retail, el dataset RPC~\cite{wei2019rpc} demostró la viabilidad del reconocimiento automático de productos en contextos de supermercado a gran escala.
...
El resto del paper está organizado de la siguiente manera: la \cref{sec:trabajos-relacionados} presenta los trabajos relacionados. La \cref{sec:metodologia} describe la metodología propuesta, incluyendo el dataset, modelo y pipeline de inferencia. La \cref{sec:experimentos} presenta los experimentos realizados y sus resultados. Finalmente, la \cref{sec:conclusiones} presenta las conclusiones y trabajo futuro.
...
Las imágenes se anotaron mediante la plataforma Roboflow~\cite{roboflow2023} con bounding boxes en formato YOLO v5 PyTorch. Se realizó una revisión manual de las anotaciones para verificar la consistencia de las etiquetas y la precisión de las cajas delimitadoras. El dataset final comprende 271 imágenes anotadas: 102 en el dataset de producto individual y 169 en el dataset multi-producto.

\subsubsection{Data Augmentation}

Se utilizaron las técnicas de data augmentation integradas en el framework Ultralytics~\cite{jocher2023ultralytics}, cuyas configuraciones por defecto han sido validadas extensivamente en la literatura de YOLO~\cite{bochkovskiy2020yolov4, shorten2019augmentation}. Las técnicas aplicadas incluyen: mosaic (probabilidad 1.0), que combina 4 imágenes de entrenamiento en una sola, incrementando la diversidad contextual; flip horizontal ($p=0.5$); y jitter de HSV (hue $= 0.015$, saturación $= 0.7$, valor $= 0.4$), que introduce variabilidad fotométrica. Adicionalmente, Roboflow aplicó auto-orientación y redimensionamiento a $640 \times 640$ píxeles con relleno por reflexión de bordes.

\subsubsection{División del Dataset}

El dataset se dividió en conjuntos de entrenamiento y validación. El dataset de producto individual contiene 102 imágenes de entrenamiento y 13 de validación. El dataset multi-producto contiene 132 imágenes de entrenamiento y 19 de validación (exportado desde Roboflow con división automática). No se definió un conjunto de test separado, lo cual se reconoce como una limitación del presente trabajo.
...
El modelo se entrenó mediante transfer learning~\cite{yosinski2014transferable} a partir de pesos pre-entrenados en COCO, utilizando el framework Ultralytics~\cite{jocher2023ultralytics}. La configuración de entrenamiento fue: 100 épocas, batch size de 16, tamaño de imagen de $640 \times 640$ píxeles, y early stopping con paciencia de 20 épocas. Se utilizó el optimizador SGD con los hiperparámetros por defecto del framework (learning rate $= 0.01$, momentum $= 0.937$, weight decay $= 0.0005$). El entrenamiento se realizó en una laptop con Apple Silicon (MPS backend) con \texttt{amp=False} para evitar incompatibilidades con la precisión mixta en dicha plataforma.
...
Se evaluó el modelo sobre un split de test independiente de 18 imágenes (no vistas durante entrenamiento ni validación), obteniendo un mAP@0.5 de 0.719, por encima del umbral mínimo de 0.70 definido en la planificación. La \cref{tab:resultados-test} presenta las métricas globales.
...
La \cref{fig:confusion-matrix} muestra la confusion matrix normalizada sobre el test split. Las clases miel y tomate alcanzan recall perfecto (1.0), mientras que leche\_entera presenta la mayor tasa de confusión con el fondo (0.67 de miss rate). Los pares aceite\_1l/aceite\_4l y leche\_descremada/leche\_entera no muestran confusión mutua significativa, lo que indica que el modelo distingue atributos de tamaño y variaciones de etiqueta. La clase fideos (mAP@0.5 = 0.581) y yerba\_kalena (0.544) requieren mayor cantidad de imágenes de entrenamiento para mejorar.
...
\begin{figure}[htbp]
  ...
  \caption{Curvas Precision-Recall por clase sobre el test split. mAP@0.5 global = 0.719.}
  \label{fig:pr-curve}
\end{figure}
...
El modelo alcanzó un mAP@0.5 de 0.865 en validación y 0.719 en el test split independiente, superando el umbral mínimo de 0.70 definido en la planificación. La diferencia entre validación y test (14.6 puntos) sugiere cierto overfitting atribuible al tamaño reducido del dataset. Si bien la comparación directa con el dataset RPC~\cite{wei2019rpc} (que reporta mAP entre 0.50 y 0.80 sobre 200 categorías) no es directa por la diferencia de escala, los resultados confirman la viabilidad del enfoque para un catálogo acotado.

\subsection{Limitaciones}

El presente trabajo tiene las siguientes limitaciones:
\begin{itemize}
  \item \textbf{Escala del dataset:} Solo se evaluaron 10 de las $\sim$60 clases del catálogo completo de la despensa, con un total de 271 imágenes anotadas.
  ...
  \item \textbf{Diferencia validación-test:} El gap de 14.6 puntos de mAP@0.5 entre validación (0.865) y test (0.719) sugiere overfitting por el tamaño reducido del dataset, mitigable con mayor cantidad de imágenes.
...
Las líneas de trabajo futuro incluyen: (1)~escalar el dataset a las $\sim$60 clases del catálogo completo, manteniendo el mismo protocolo de captura y augmentation; (2)~generar un split de test formal y métricas de sistema completas; (3)~evaluar el rendimiento bajo condiciones de iluminación variable y fondos no controlados; (4)~desplegar el sistema en Raspberry Pi~5 para validación en condiciones reales de operación; y (5)~integrar el módulo de detección con el sistema de gestión de la mutual para automatizar el cobro.

%% ===========================================================================
\section{Conclusiones}
\label{sec:conclusiones}

Se presentó un sistema de detección y clasificación de productos en tiempo real para autocobro en despensas comunitarias, basado en fine-tuning de YOLOv10n sobre un dataset propio de 10 clases. El sistema alcanzó un mAP@0.5 de 0.924 en escenas multi-producto con inferencia superior a 15~FPS, superando los umbrales mínimos definidos para el proyecto.

La lógica de registro multi-frame basada en IoU demostró ser efectiva para suprimir falsos positivos sin introducir latencia perceptible, ofreciendo una alternativa simple y robusta frente a trackers más complejos para el caso de uso de productos estacionarios.

La metodología propuesta es reproducible y extensible: el mismo protocolo de captura, pipeline de augmentation y arquitectura de modelo pueden aplicarse para escalar el catálogo a las $\sim$60 clases completas de la despensa. El código fuente y los datasets se encuentran disponibles públicamente para facilitar la replicación y extensión del trabajo.

Los resultados validan la viabilidad de un sistema de autocobro basado exclusivamente en visión por computadora como alternativa de bajo costo para organizaciones comunitarias, sin requerir sensores adicionales ni infraestructura de nube.
